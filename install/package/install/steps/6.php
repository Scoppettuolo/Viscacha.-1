...
if (extension_loaded('pdo_sqlite') == true) {
	$sqliteext = '<span style="color: green;">Available</span>';
	if ($select == '') $select = 'sqlite';
}
else {
	$sqliteext = '<span style="color: red;">Unavailable</span>';
}
	...
	<table class="tables">
	 <tr>
	  <td class="mbox" width="50%">
	  	Database Driver:<br />
	  	<span class="stext">
	  	SQLite (pdo_sqlite): <?php echo $sqliteext; ?><br />
	  	<a href="http://www.php.net/manual/ref.mysqli.php" target="_blank">mysqli</a>: <?php echo $mysqliext; ?><br />
	  	<a href="http://www.php.net/manual/ref.mysql.php" target="_blank">mysql</a>: <?php echo $mysqlext; ?><br />
	  	<span class="stext">For SQLite put the database <b>file path</b> in "Database Name" (e.g. data/viscacha.db). Host/user/password are ignored.</span>
	  	</span>
	  </td>
	  <td class="mbox" width="50%">
	  <select name="dbsystem">
	  	<option value="sqlite"<?php echo iif($select == 'sqlite', ' selected="selected"'); ?>>SQLite (recommended for simple installs)</option>
	 ...
	 <tr>
	  <td class="mbox" width="50%">Database Name / SQLite file:<br /><span class="stext">MySQL: database name. SQLite: path to .db file (e.g. data/viscacha.db)</span></td>
	  <td class="mbox" width="50%"><input type="text" name="database" value="<?php echo !empty($config['database']) ? $config['database'] : 'data/viscacha.db'; ?>" size="50" /></td>
	 </tr>
	 <tr>
